...
usertrap(void)
{
  ...
  if(r_scause() == 8){
    ...
    char *pa;
    // 获取发生错误的虚拟地址并向下取整到页面边界
    uint64 va = PGROUNDDOWN(r_stval());
    struct VMA *vma = 0;
    int flags = PTE_U;// 设置初始的权限标志为用户可访问
    int i;
     // 查找当前进程的 VMA 数组中是否存在与该地址相关的 VMA 结构
    for (i = 0; i < NVMA; ++i) {
      // 检查该地址是否在某个 VMA 的范围内
    ...
    if (r_scause() == 15 && (vma->prot & PROT_WRITE)
      ...
      // 对于写操作的 page fault，设置脏页标志位 (PTE_D)
      if (uvmsetdirtywrite(p->pagetable, va)) {
        goto err;
      }
    } else {
      // 对于其他类型的页错误（如读操作），执行延迟分配
      if ((pa = kalloc()) == 0) {
        goto err;
      }
      memset(pa, 0, PGSIZE);// 清空分配的物理页
      ilock(vma->f->ip);
